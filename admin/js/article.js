    ...
    $('#article_add').on('click', function () {
        ...
        layer.open({
            ...
            yes: function (index) {
                ...
                _request('operate/addArticle.php', 'title=' + title + '&content=' + content, (xhr) => {
                    layer.close(index);
                    edit_title.val('')
                    edit_content.val('')
...
function article_view(obj) {
    ...
    $('#view_title').text(DT.cell(_row, 2).data())
    $('#view_content').html('<p style="text-indent:2em">' + DT.cell(_row, 3).data().replaceAll('\n', '</p><p style="text-indent:2em">') + '</p>')
    $('#view_time').text(DT.cell(_row, 4).data())
    layer.open({
        type: 1,
        title: '文章信息',
        shadeClose: false,
...
function article_edit(obj) {
    ...
    let id = DT.cell(_row, 1).data()
    $('#edit_id').text(id)
    $('#edit_time').text(DT.cell(_row, 4).data())
    let edit_title = $('#edit_title')
    let edit_content = $('#edit_content')
    let originTitle = DT.cell(_row, 2).data()
    let originContent = DT.cell(_row, 3).data()
